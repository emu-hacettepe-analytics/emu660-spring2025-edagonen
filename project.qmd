...
# Top 10 Movies Rating Bar Chart (Corrected Version)
movies %>%
  filter(rating > 0) %>%                     # IMDb puanı olan filmleri al
  arrange(desc(rating)) %>%                  # Puanlara göre azalan sırala
  slice_head(n = 10) %>%                     # En yüksek 10 filmi seç
  ggplot(aes(x = reorder(title, rating), y = rating, fill = factor(year))) +  # Renk farklılığı için yıl kullan
  geom_col(show.legend = FALSE) +             # Kolon grafiği
  coord_flip() +                              # Yatay başlıklar
